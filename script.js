// View Team Dashboard PRO v2.6 (final header + menu)
...
function teamGoalStats(){
  let sumRev=0,sumGoal=0;
  Object.keys(USERS).forEach(email=>{
    if(USERS[email].role!=='sales') return;
    const d=DATA.users[email]||{};
    const g=USERS[email].goal||0;
    if(g>0){ sumRev += (d.revenue||0); sumGoal += g; }
  });
  const pct = sumGoal? Math.round((sumRev/sumGoal)*100) : 0;
  return {sumRev,sumGoal,pct};
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('loginBtn').addEventListener('click',()=>{
    ...
    // role in header (show manager-only link)
  ...
  document.getElementById('mainNav').addEventListener('click',(e)=>{
    ...
    if(route==='reporty'){ openReport(); return; } // open modal via menu
    navigate(route);
  });
  // Report shortcut
...
/* Navigate to route and render views by role */
function navigate(route){
  ...
  if(route==='dashboard'){
    ...
    if(cur.role==='manager'){
      ...
      renderSales(cur.email);
    }
  }else if(route==='ciele'){
    showRoute('ciele');
    renderGoalsTable();
  }else if(route==='obchodnici'){
    showRoute('obchodnici');
    renderManager(); // reuse to show team list etc.
  }else{
    showRoute('dashboard');
  }
}
